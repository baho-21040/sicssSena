...
export default function EditarPerfilAprendiz() {
    ...
    const handleActualizarCorreo = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API}/api/aprendiz/actualizar-correo`, {
                method: 'PUT',
                headers: {
    ...
    const handleCambiarContrasena = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API}/api/aprendiz/cambiar-contrasena`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                    'Authorization': `Bearer ${token}`
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            {perfilData?.programa && (
                                <>
                                    ...
                                    <div className="bg-gray-50 p-4 rounded-lg">
                                        <label className="block text-sm font-semibold text-gray-600 mb-2">
                                            <i className="fas fa-hashtag mr-2"></i>Número de Ficha
                                        </label>
                                        <p className="text-lg font-semibold text-gray-800 break-words">
                                            {perfilData.programa.numero_ficha}
                                        </p>
                                    </div>
                                </>
                            )}
